...
// Télécharger le PDF d'un contrat (généré côté serveur avec Puppeteer)
window.downloadContratPDF = async (contratId) => {
    try {
        // Ouvrir le PDF généré par l'API serveur dans un nouvel onglet
        window.open(`/api/contrats/${contratId}/pdf`, '_blank');
        showMessage('PDF en cours de téléchargement', 'success');
    } catch (error) {
...
window.downloadQuittancePDF = async (quittanceId) => {
    try {
        // Ouvrir le PDF généré par l'API serveur dans un nouvel onglet
        window.open(`/api/quittances/${quittanceId}/pdf`, '_blank');
        showMessage('PDF en cours de téléchargement', 'success');
    } catch (error) {
        console.error('Erreur lors du téléchargement du PDF:', error);
        showMessage('Erreur lors du téléchargement du PDF', 'error');
